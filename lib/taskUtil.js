...
exports.pauseTask = function(taskID) {
	...
	queryAdapter.query(sql).then(function(results) {
		...
		{
			//console.log(">>>>>>>>>>>>>>>>>>in pauseTask 3<<<<<<<<<<<<<<<<<<<<<<<");
			if (results[0].taskStatus == taskStatus["awake"] && results[0].timerID != null)
			{
				timerManager.cancelTimer(results[0].timerID);
			}
			//console.log(">>>>>>>>>>>>>>>>>>in pauseTask 4<<<<<<<<<<<<<<<<<<<<<<<");
...
exports.hastenTask = function(taskID,isNow,hasten)
{
	...
	queryAdapter.query(sql).then(function(results) {
		...
		else
		{
			...
			{
				//console.log(">>>>>>>>>>>>>>>>>>in hastenTask2<<<<<<<<<<<<<<<<<<<<<<<");
				if (results[0].taskStatus == taskStatus["awake"] && results[0].timerID != null)
				{
					timerManager.cancelTimer(results[0].timerID);
				}
				//console.log(">>>>>>>>>>>>>>>>>>in hastenTask3<<<<<<<<<<<<<<<<<<<<<<<");
				var timerID = null;
				var newD = (isNow ? new Date():dateUtil.dateAdd(dateUtil.utcDateRefine(results[0].executeDate),-hasten));
				//console.log(">>>>>>>>>>>>>>>>>>in hastenTask4<<<<<<<<<<<<<<<<<<<<<<<");
				var evaluatedResult = evaluateTaskTime(newD);
				//console.log(">>>>>>>>>>>>>>>>>>in hastenTask5<<<<<<<<<<<<<<<<<<<<<<<");
				if (evaluatedResult.status == taskStatus["awake"] || evaluatedResult.status == taskStatus["run"])
				{
					results[0].executeDate = newD;
					results[0].taskStatus = taskStatus["awake"];
					timerID = timerManager.assignTimer(hitch(doTaskByTimer,results[0]), evaluatedResult.timeDiff==null?5000:evaluatedResult.timeDiff);
				}
				//console.log(">>>>>>>>>>>>>>>>>>in hastenTask6<<<<<<<<<<<<<<<<<<<<<<<");
				sql = 'UPDATE fixtimetask SET taskStatus = $status, executeDate = $date1, timerID = $timerID WHERE taskID = $taskID'
				.replace('$status',evaluatedResult.status).replace('$date1',dateUtil.dateToString(newD))
				.replace('$timerID',timerID).replace('$taskID',taskID);
				queryAdapter.query(sql).then(function(results2) {
					//console.log(">>>>>>>>>>>>>>>>>>in hastenTask7<<<<<<<<<<<<<<<<<<<<<<<");
	...
	queryAdapter.query(sql).then(function(results) {
		...
		else
		{
			...
			else
			{
				...
				sql = 'UPDATE fixtimetask SET taskStatus = $status, executeDate = $date1, timerID = $timerID WHERE taskID = $taskID'
				.replace('$status',evaluatedResult.status).replace('$date1',dateUtil.dateToString(newD)).replace('$timerID',timerID).replace('$taskID',taskID);
				queryAdapter.query(sql).then(function(results2) {
...
exports.changeFrequency = function(taskID,frequency,intervalTime) {
	var defer = Q.defer();
	...
	{
		var interval = (frequency == 'fixinterval'?(', intervalTime = '+intervalTime):'');
		var sql = 'UPDATE fixtimetask SET fequency = $frequency $interval WHERE taskID = $taskID'
		.replace('$taskID',taskID).replace('$frequency',taskFrequecy[frequency]).replace('$interval',interval);
		queryAdapter.query(sql).then(function(results) {
